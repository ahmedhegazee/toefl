<template>
  <div>
    <b-form-textarea id="textarea-plaintext" plaintext :value="description" rows="10" no-resize></b-form-textarea>
    <div class="clearfix mb-2 mt-2">
      <b-img
        center
        thumbnail
        :src="img"
        style="max-width: 70vw;border-width: 3px !important;"
...
export default {
  ...
  mounted() {
    if (this.isGrammar == "true") {
      ...
      this.description =
        "To add multiple questions .You have to follow specific format.\n" +
  ...
  methods: {
      ...
      if (this.isGrammar == "true") {
        for (var i = 0; i < questions.length; i++) {
          if (questions[i].length == 0) {
            ...
            this.alert.message.push(
              "write correct question which its id is " +
                (i + 1) +
                " using specific format please  "
            );
            var question = {
              id: i + 1,
              question: "",
            ...
            if (answers.length < 7) {
              this.alert.show = true;
              this.alert.message.push(
                "write correct options in the question with id " + (i + 1)
              );
            } else if (
              answers[6].length == 0 ||
              answers[6] > 4 ||
              ...
              this.alert.show = true;
              this.alert.message.push(
                "write correct answer in the question with id " + (i + 1)
              );
            } else if (
              answers[1].length == 0 ||
              answers[1] > 2 ||
              ...
              this.alert.show = true;
              this.alert.message.push(
                "write correct question type in the question with id " + (i + 1)
              );
            }
            var question = {
              id: i + 1,
    ...
    storeQuestions() {
      if (!this.alert.show) {
        this.$bvModal
          .msgBoxConfirm("Are you sure about storing these questions ", {
        ...
        this.$bvModal
          .msgBoxOk("Please check the errors for correction them", {
            title: "Attention",
            size: "sm",
            buttonSize: "sm",
            okVariant: "primary",
